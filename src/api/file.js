...
export const sysfiledirAcionEdit = data => request({
  url: '/api/v1/sys-file-dir/' + data.id,
  method: 'PUT',
  data
})

export const sysfiledirAcionDel = data => request({
  url: '/api/v1/sys-file-dir/' + data,
  method: 'DELETE'
})

export const sysfileinfoList = data => request({
  url: `/api/v1/sys-file-info?pId=${data.pId}&pageSize=${data.pageSize}&pageIndex=${data.pageIndex}`,
  method: 'GET',
  data
})

export const sysfileinfo = id => request({
  url: '/api/v1/sys-file-info/' + id,
  method: 'GET'
})

export const sysfileinfoAdd = data => request({
  url: '/api/v1/sys-file-info',
  method: 'POST',
  data
})

export const sysfileinfoEdit = data => request({
  url: '/api/v1/sys-file-info/' + data.id,
  method: 'put',
  data
})
