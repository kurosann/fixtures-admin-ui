...
<script>
import { delSysContent, listSysContent } from '@/api/admin/app-content'
import { listSysCategory } from '@/api/admin/app-category'
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({ name: 'AppContentCreate' })
    },
    // 多选框选中数据
    ...
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.$router.push({ name: 'AppContentEdit', params: { id: row.id }})
    },
    /** 删除按钮操作 */
    handleDelete(row) {
